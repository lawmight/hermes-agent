...
from typing import Optional, List, Dict, Any, ClassVar, Iterator, Tuple
...
class ShellFileOperations(FileOperations):
    ...
    def _search_files(self, pattern: str, path: str, limit: int, offset: int) -> SearchResult:
        ...
        if self._has_command('rg'):
            ...
            if self._has_command('find'):
                # Keep going into the find path below.
                pass
            else:
                return result
    ...
    def _search_files_rg(self, pattern: str, path: str, limit: int, offset: int) -> SearchResult:
        ...
        # `set -o pipefail` so a broken rg (e.g. exit 126) is not masked by head.
        cmd_sorted = (
            f"set -o pipefail; "
        ...
        if result.exit_code not in (0, 1) and not all_files and not limit_reason:
            return SearchResult(
                error=f"Search failed: rg --files exited {result.exit_code}",
                total_count=0,
            )
    ...
    def _search_content(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        def _finish(result: SearchResult, used_rg: bool = False) -> SearchResult:
            ...
            # as a warning. Runs for every engine.
            ...
            # explanation only applies to the non-rg engines.
            if used_rg:
                return result
        ...
        if self._has_command('rg'):
            ...
            if result.error is None:
                return _finish(result, used_rg=True)
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # otherwise matched), so only surface an error when the exit is not a
        # normal search completion (0/1) AND no usable match payload remains.
        # That also covers broken binaries (126/127) so LocalEnvironment can
        # fall back to stdlib search.
        if result.exit_code not in (0, 1) and not payload.strip():
    ...
    def _search_with_grep(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # other files matched, so only surface an error when the exit is not a
        # normal search completion (0/1) AND no usable match payload remains.
        if result.exit_code not in (0, 1) and not payload.strip():
            error_msg = diagnostics.strip() or result.stdout.strip() or "Search error"
            return SearchResult(error=f"Search failed: {error_msg}", total_count=0)
