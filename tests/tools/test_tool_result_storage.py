...
class TestMaybePersistToolResult:
    ...
    def test_tool_use_id_cannot_escape_storage_dir(self):
        ...
        storage = STORAGE_DIR.replace("\\", "/")
        assert f"Full output saved to: {storage}/outside_whoami_x_" in result.replace("\\", "/")
        assert f"{storage}/../" not in result.replace("\\", "/")
        assert target.replace("\\", "/").startswith(f"{storage}/outside_whoami_x_")
